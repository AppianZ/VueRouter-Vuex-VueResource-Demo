<template>
	<div @click.stop="" @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}"
		...
		<div class="calendar-tools">
			...
			<div class="text center">
				<input type="number" v-model="year" value="{{year}}" @change="render(year,month)" min="1970" max="2100"
					   maxlength="4" number>
				/
				{{month+1}}
			</div>
	...
	module.exports = {
		...
		created() {
			...
			if (that.value != "") {
				...
				if (that.range) {
					const split = that.value.split(" ~ ");
					if (split.length > 1) {
						const beginSplit = split[0].split(that.sep);
						const endSplit   = split[1].split(that.sep);
		...
		methods: {
			...
			render(y, m) {
				...
				const seletSplit         = that.value.split(" ")[0].split(that.sep);
				let line                 = 0, temp = [];
				for ( let i = 1; i <= lastDateOfMonth; i++ ) {
					...
					if (that.range) {
						...
						if (that.rangeBegin.length > 0) {
							const beginTime = Number(new Date(that.rangeBegin[0], that.rangeBegin[1], that.rangeBegin[2]));
							const endTime   = Number(new Date(that.rangeEnd[0], that.rangeEnd[1], that.rangeEnd[2]));
							const thisTime  = Number(new Date(that.year, that.month, i));
